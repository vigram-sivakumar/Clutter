...
- **Engagement mechanics reconcile "atomic" and "arrow-key-enterable."** *Locked.* Passing through from a distance is atomic (one hop, like a single character); a *second* press once the caret is already at the boundary steps inside. Same mechanism at two distances, not two mechanisms. Felt-experience of this gesture is flagged for prototype confirmation, not the mechanism itself.
