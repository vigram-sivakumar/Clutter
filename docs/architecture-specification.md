...
- `save()`, `updateDraftTitle()`, and `updateMetadata()` can each promote a draft to a real page — any committed, persistent, user-owned change (body, title, or an editable metadata field) does, per [ADR-017](./adr/017-draft-page-lifecycle.md)'s amendment — all three check `vault.getPage(pageId)`/`this.drafts.has(pageId)` and dispatch through the same private helper (`persistDraft`, extended with an optional metadata patch) `create()` uses, never a duplicated "resolve path, enqueue create" implementation. Each check is an optimistic guess about which Gate call to make, not a correctness guarantee — see §5's `create` invariant for where correctness against a racing second draft-promotion actually lives. Daily Note drafts (`deterministicPath` set) participate via `save()`'s body trigger and, per [ADR-017](./adr/017-draft-page-lifecycle.md)'s second amendment (Cover Image milestone), `updateMetadata()`'s trigger too, on a genuine committed metadata change — only `updateDraftTitle()` remains excluded, since a Daily Note's title is derived from its date and is never user-committed.
