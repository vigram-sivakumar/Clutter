...
**Last verified:** 2026-08-21, against commit `809a9909` (branch `markdown`). Superseded parts of the original audit are marked "**Updated**" below.
...
|---|---|---|---|---|---|---|
| Bold `**text**` | ✓ | ✓ | ✓ | Implemented | A | — |
| Italic `*text*` | ✓ | ✓ | ✓ | Implemented | A | — |
| Strikethrough `~~text~~` | ✓ | ✓ | ✓ | GFM extension + marker decoration + compact renderer | A | — |
| Inline code `` `code` `` | ✓ | ✓ | ✓ | Implemented | A | — |
...
| WikiLink `[[Page]]`, `[[Page\|alias]]` | ✓ | – | – | Full grammar, resolution, autocomplete, widget, compact rendering | A | — |
| Tag `#tag` | ✓ | – | – | Full grammar, resolution, autocomplete, widget | A | — |
| Date `@2026-08-21` | plugin | – | – | Clutter-specific `@`-family token, full lifecycle | A | — |
| Highlight `==text==` | ✓ | bg color | ✓ | **Updated — now implemented**: `highlightSyntax` grammar node + `highlightMarkerDecoration` + compact renderer (commit `9932a06e`) | A | — |
...
| Blockquote `> text` | ✓ | ✓ | ✓ | **Updated — now implemented**: `blockquoteMarkerDecoration` (commit `f8111f61`) | A | — |
| Bullet / ordered lists | ✓ | ✓ | ✓ | **Updated — now implemented**: `listMarkerDecoration` + `ListBulletWidget` render resting markers as a styled glyph (commits `f8111f61`, `824b3b66`, `809a9909`) | A | — |
| Task checkbox `- [ ]` / `- [x]` | ✓ | ✓ | ✓ | **Updated — now implemented**: GFM `TaskList` node + `TaskCheckboxWidget`, interactive/clickable in-editor, wired to the Tasks feature (commits `7d2a22b7`, `dbac5aa1`) | A | — |
...
| Feature | Obsidian | Notion | Craft | Clutter | Cat. | Layer (if B) |
|---|---|---|---|---|---|---|
| Reveal-on-engagement Live Preview | ✓ | n/a — block model | n/a — block model | Locked architectural pattern, shared across all semantic tokens | A | — |
| Autocomplete for links/tags/dates | ✓ | ✓ | limited | Full completion sources for WikiLink, Tag, Date | A | — |
| Typing shortcuts `**` →bold, `-` →list, `>` →quote, `---` →divider | ✓ | ✓ | ✓ | **Verified 2026-08-21 in the running app**: `**bold**`, list markers, and `> quote` all already reveal styled the instant the caret leaves them — a direct consequence of the marker decorations shipped for those constructs (rows above), not a dedicated autoformat layer. Remaining real gap: no keyboard-shortcut path (Cmd/Ctrl-B, -I, -E) to *apply* bold/italic/code to a selection without hand-typing the marker pair; `---` also still only renders as a rule after the fact, no live divider-on-type | B | editor keymap/interaction, tests |
