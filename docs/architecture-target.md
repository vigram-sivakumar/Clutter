...
> **Executed differently than described above — see [ADR-013](./adr/013-phase3-move-backend-and-presentational-dedup.md).** Item 9's "one-line facade addition" undersold the actual gap: it needed a new `MoveService` method, a new `move` Gate kind, and only then the one-line `PageOperations.move()` facade method — three commits, not one. Its "wire a 'Move to…' UI entry point" half was not done: no destination-picker component exists anywhere in the codebase, so a repo-wide search confirmed the note topbar menu was the only reference to a Move trigger, and that item was removed rather than left inert, pending a future phase that builds the picker. Items 10–11 shipped as written. Item 12 shipped in full; the page-model unification needed no parameterization by resource type — the two files turned out identical except a dead `rename()` stub, which was removed (zero callers anywhere, confirmed including the title-edit component). Read the ADR before treating this phase's original text as an accurate description of what shipped.
...
> **Executed differently than described above — see [ADR-014](./adr/014-phase4-composition-root-and-navigation-cleanup.md).** Item 13's premise (two `ResourceCreation`s etc.) doesn't apply — those files never existed on this branch (ADR-011) — but the split itself was real, larger work than "explicit `bootstrap()`/`attachVault()`": the frozen spec's own Startup sequence turned out to be internally inconsistent (a "minimal Gate" running before the Vault exists, contradicting §5's own invariant that the Gate requires a `Vault`), which required an actual spec amendment, not just an implementation choice — see the spec's Startup sequence and Composition Root Lifecycle sections. Item 14 shipped exactly as described. Item 15's 8 stubs split 6-vs-2: the 6 spec-§8 view-intent methods were deleted (all 6 shared one blocker — `Workspace` has no "active view" state to render into, a bigger gap than a Phase 4 cleanup item), and `createTask`/`createTag` were left unchanged per ADR-012's existing, still-blocked disposition. Read the ADR before treating this phase's original text as an accurate description of what shipped.
...
> **Executed differently than described above — see [ADR-016](./adr/016-phase6-cleanup-and-migration-close.md).** Items 18-19 shipped exactly as described. Item 20's "alias" was dropped — no vault-wide alias projection exists to make lazy — and making `knowledgeGraph`/`embeds` lazy required a real method-shape fix (`knowledgeGraph` was a property getter, spec §3 wants a method) and splitting `VaultProjectionBuilder.build()` into eager/lazy halves, not just a timing change. Item 21 split: `References`' fake wiring (rendered with no data source anywhere) was removed, but the presentation component itself was kept — clean, reusable, and now a deliberately deferred feature pending a backlink-indexing subsystem and `VaultQuery` exposure, not dead code. `Controls` was kept in full, with all 3 controls (including a previously-enabled sidebar-toggle with no handler at all) made honestly `disabled` as placeholders for navigation-history and sidebar-collapse features that don't exist yet — resolving neither "wire" nor "delete" literally, but closing the same underlying no-silent-no-op concern. A broader sweep found 9 more enabled no-op buttons elsewhere in the app sharing that same defect, outside this migration's scope, added to the backlog rather than fixed here. **The six-phase migration plan is now complete** — every item has a disposition, though several architectural questions this migration surfaced (including that 9-button sweep) remain deliberately unimplemented, not undiscovered. Read the ADR before treating this phase's original text as an accurate description of what shipped.
