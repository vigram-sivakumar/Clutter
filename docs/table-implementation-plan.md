...
**Status:** Planning only. No production code has been written or changed. This document is the blueprint to follow before the first production change; it does not itself authorize implementation — each milestone in §E still needs its own `tsc`/test/commit discipline per the project's standing workflow.
...
- **Existing implementation being replaced:** all seven files under `codemirror/table/` except `tableGeometry.ts`/`tableAlignment.ts` (retained), and their seven test files — full inventory in §B.
