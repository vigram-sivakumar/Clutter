...
**What enters this stage:** a user-authored or app-initiated edit (title or body) that has been committed to a `DocumentSession` via `DocumentSession.commit(transaction)`, producing a new immutable `DocumentRevision`. Per [ADR-031](./adr/031-app-initiated-body-mutation-routing.md), a body mutation the app itself computes and applies — task completion/reopening, a formatting command, a bulk transform, an AI-assisted edit — enters at this exact stage, through this exact call, whenever `PageOperations.mutateBody()` finds an open session for the target page: it calls `PageOperations.commitEdit(pageId, transform(session.currentRevision.markdown))` — the identical method (and therefore the identical `commit()` call plus autosave-arming `SaveCoordinator.scheduleSave()` step) a keystroke-originated edit already goes through. There is no separate stage, guarantee, or persistence trigger for a programmatic commit versus a typed one. Content sitting in an unfocused, uncommitted editor buffer (raw keystrokes not yet blurred/committed) has not yet entered this stage at all — it exists only in the DOM, with no guarantee whatsoever.
