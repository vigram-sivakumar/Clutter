...
The 8 remaining throwing stubs on `NavigationService` split into two categories with different fates per spec §8, not one undifferentiated bucket:

- **Already-decided removals:** `createTask`, `createTag` — spec §8's `NavigationRouter` public API excludes these entirely, the same way it excluded `createNote` ("callers use `PageOperations.open`/`.create` directly, since those methods added no logic beyond forwarding"). Their eventual fate is deletion, not implementation — identical to `createNote`'s. They are not deleted in Phase 2 only because, unlike `createNote`, Phase 2 never touched the Tasks/Tags UI: `buildTasksShortcutHandler.ts`/`buildTagsShortcutHandler.ts` still call them live, and no `TaskOperations`/`TagOperations` facade exists yet to redirect them to. Deleting them now would break those call sites with no replacement — building `TaskOperations`/`TagOperations` is unambiguously out of Phase 2's scope (a new aggregate, not a consolidation of existing services).
- **Genuine product-decision navigation methods:** `openFavorites`, `openAllNotes`, `openAllTasks`, `openSomedayTasks`, `openCompletedTasks`, `openAllTags` — these *are* part of spec §8's `NavigationRouter` surface (view-level intents, not aggregate operations). Whether/how they ship is a product priority call, per the target doc ("implement or delete... based on actual product priority — this is a product decision, not an architecture one"), deferred to Phase 4.

`createNote()` was deleted immediately in commit 5, ahead of `createTask`/`createTag`, because commit 5's rewiring of `buildNotesShortcutHandler.ts` to call `PageOperations.create()` directly made the stub provably dead code as a direct consequence of that same commit — the compliance checklist requires removing dead code a change produces, not leaving it alongside its replacement. `createTask`/`createTag` will follow the identical path once `TaskOperations`/`TagOperations` exist and their shortcut handlers are rewired to call them directly.

**Disposition:** `createNote` — permanent, already reached. `createTask`/`createTag` — permanent removal, blocked on `TaskOperations`/`TagOperations` existing (no phase assigned). The other 6 — Phase 4, product decision.
