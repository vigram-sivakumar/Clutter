...
## Amendment (implementation sequencing): defer `move` until the Folder Picker UI exists

Raised before implementation began: the frontend is under active redesign and does not yet have a Folder Picker dialog, a Move modal, destination-selection UX, or destination-validation UX. Implementing `FolderOperations.move()`/the app-initiated move write path now would have no UI that could exercise it. This is an **implementation-sequencing decision, not an architectural one** — §4's original design (one unified `'move-folder'` Gate kind for both move and rename, since `Vault.moveFolder()` is already one method for both) remains the target end state and is not being replaced. This amendment records how that target state is reached in two steps instead of one, without the interim step becoming a second, permanent kind of fragmentation.

**Deferred, not implemented in this phase:** `FolderOperations.move()`, the unified `'move-folder'` `PersistenceOperation` kind as originally specified in §4/§5, the Folder Picker dialog, destination-selection UX, destination-validation UX, and any other frontend move workflow.

**Implemented now, as an explicitly interim, narrower kind:** `FolderOperations.rename(folderId: string, name: string): Promise<void>`, backed by a new `'rename-folder'` `PersistenceOperation` kind — `{ readonly kind: 'rename-folder'; readonly name: string }` — scoped to a same-parent path change only (it never reparents; there is no `destinationFolderId` in its shape at all, so it cannot express a move even by caller error). `runRenameFolder` resolves a collision-free destination path under the folder's *current* `parentId` (a new `FolderPathResolver.resolveRenamePath` method, reusing `resolveCollisionFreeName` exactly like `createFolderPath` already does), then calls `fileSystem.moveFile()` and `vault.moveFolder(folderId, newPath, currentParentId)` — the same already-correct, already-cascading Vault primitive §5 always intended to reuse. Nothing about `Vault.moveFolder()`, its cascade, or its notification shape changes; only the Gate's operation-kind vocabulary is temporarily narrower than §4's target.

**This is a time-boxed interim, not a second permanent write path.** When `FolderOperations.move()` ships (Folder Picker UI landing), `'rename-folder'` must be retired and merged into the unified `'move-folder'` kind exactly as §4 originally specified, with `FolderOperations.rename()` switching to call the unified kind (passing the folder's existing `parentId` as `destinationFolderId`). This retirement is required cleanup at that time, not optional — leaving both kinds live afterward would be exactly the "two Gate kinds for one Vault operation" fragmentation Alternative A below already rejects. Tracked here so it isn't forgotten once the picker UI exists.

**External rename and external move are not deferred, and cannot be separated.** `VaultSyncService`'s folder-`handleMoved` branch (§6) calls `vault.moveFolder()` directly — Sync never goes through the Persistence Gate or its `PersistenceOperation` vocabulary at all (confirmed: existing page `handleMoved` already calls `vault.updatePagePath()` directly, no Gate involvement), so the `'move-folder'` deferral above does not restrict it. More fundamentally, the filesystem watcher cannot distinguish an external rename from an external move — both a renamed and a reparented directory arrive as exactly one `moved` event (`{fromPath, toPath}`, confirmed in this ADR's Phase 1 review: `vault_watcher.rs` has no `is_dir` branching and no rename/move distinction of any kind), and `Vault.moveFolder()` itself doesn't care which one occurred. Implementing "external folder rename synchronization" therefore *is* implementing "external folder move synchronization" — there is one code path, not two, and it ships in this phase. This is not part of the deferred *workflow* (the in-app Folder Picker and its Gate-level write) — it is passive reconciliation of a fact that already happened on disk, exactly parallel to how external page moves already reconcile today with no in-app "Move" UI for pages existing either.

**Folder archive is confirmed out of scope for this ADR, not merely deferred.** `folderTopBarMenu.config.ts`'s existing comment already states this independently of ADR-024: *"'archive' and 'move-to' are deliberately absent: FolderOperations has no archive concept in the frozen spec at all (a permanent impossibility, not a deferred one)."* `Folder.metadata` does structurally carry `status`/`archivedAt`/`originalPath`/`originalParentId` fields (mirroring `PageMetadata`'s shape), but no business rule, facade method, or Gate operation kind exists anywhere for using them, and none is introduced here. If folder archiving becomes a real product requirement, it needs its own ADR — this one does not silently imply it.

## Amendment (delete invariant correction): physical subtree deletion replaces tracked-descendant enumeration

§5 step 4's original justification for needing no recursive-delete Platform capability was: *"by construction, by the time an ancestor directory is deleted, every path still inside it has already been removed"* — i.e., the bottom-up cascade over `Vault.getDescendantFoldersAndPages()` was assumed to leave each directory physically empty before its own non-recursive `deleteFile()` call.

**This premise was empirically false.** `Vault.getDescendantFoldersAndPages()` only enumerates Vault-*tracked* pages and folders. Physical directory contents are not guaranteed to equal that set: OS artifacts (`.DS_Store`, editor swap files), files dropped in by external tools, and — notably — `Assets/`'s own image files (`Assets` is ordinary, unreserved, user-visible vault content, populated by a documented non-Gate write path in `importAsset.ts`) can all be physically present without ever becoming a tracked `Page`. Any folder containing such content threw `ENOTEMPTY` (os error 66) on delete, with no rollback, leaving the folder's `.folder.md` deleted but its directory and Vault record still present — an inconsistent, silently-corrupted state, not just a failed no-op.

**Corrected invariant:** permanently deleting a folder removes its entire physical subtree — tracked or not — via one recursive `VaultFileSystem.deleteFile(folder.path, { recursive: true })` call, strictly scoped to the folder's own path, followed by `Vault.removeFolder()`'s existing full in-memory cascade (unchanged — it already reconciles every descendant `Page`/`Folder` record independent of disk state). `PagePersistenceCoordinator.runDeleteFolder()` no longer enumerates descendants for the disk operation at all; `getDescendantFoldersAndPages()` remains in use elsewhere (`FolderOperations.delete()`'s pre-enqueue session cleanup, `Vault.removeFolder()`'s own internal cascade) for its original, still-valid purposes.

**This directly revisits Alternative B below**, previously rejected. That rejection was conditioned on the bottom-up and recursive approaches being "equally correct" — a premise this bug disproves outright, not merely a preference to reconsider. Its two stated concerns are addressed, not overridden: (1) the Platform-interface-minimalism concern is satisfied by extending the *existing* `deleteFile` primitive with an additive `{ recursive?: boolean }` option rather than introducing a new `deleteDirectory` method — no new abstraction, no new write path, rule 1/4 unaffected; (2) the crash-atomicity concern (a crash mid-cascade leaving a partial deletion) applies equally to the old per-descendant loop, which was never atomic either — a crash between two `deleteFile()` calls in the old cascade left exactly the same kind of partial state a crash mid-recursive-delete would. Recursive deletion does not trade away an atomicity guarantee the bottom-up cascade actually provided; it simply also closes the untracked-content gap the bottom-up cascade never covered.

§5 step 4's original text above is left as the historical record of the initial (incorrect) design; this amendment is the current authority for the `delete-folder` operation kind's behavior.
