...
Notably, this is not merely an unresolved product ambiguity — it is a **drift from an existing, accepted ADR's stated design**. [ADR-022](./022-workspace-favorites-active-view.md) §"How they fit the existing active-view model" states explicitly that the Workspace collection page builds its model from `VaultQuery.getRootFolders()`/`getRootPages()`, describing this as *"the same query methods the sidebar's `FolderTree`/`FavoriteList` already call."* That claim is false as currently implemented: `FolderTree` calls `getVisibleRootFolders()` (filtered), not `getRootFolders()` (unfiltered). Either `FolderTree` diverged from ADR-022's design after that ADR was accepted, or the two were never actually unified when ADR-022 shipped. Either way, ADR-022 already recorded parity as the intended state — this ADR does not reopen that as a fresh product question; it treats the sidebar's filtered read as a **conformance gap against ADR-022**, and assigns the Membership Selector as the one place that parity is enforced going forward, so the same drift cannot recur silently a second time.
...
`VaultQuery`'s contract is also unchanged. `getVisibleRootFolders()`'s existing `RESERVED_FOLDER_NAMES` filter remains available as a structural primitive; what changes is that it stops being treated as an independent second answer to "what is Workspace" alongside `getRootFolders()`. Per ADR-022's own stated design (unfiltered `getRootFolders()`/`getRootPages()`, explicitly described there as shared with the sidebar), this ADR treats ADR-022's answer as already decided — the Membership Selector's Workspace-membership implementation is the single place that answer is expressed, and both the sidebar's `FolderTree` and the Workspace collection page consume it, closing the drift identified in the Context section rather than reopening which answer is correct.
...
- **Does not relitigate ADR-022's Workspace/Favorites design.** ADR-022 already specifies unfiltered `getRootFolders()`/`getRootPages()` as the intended Workspace-membership answer. This ADR does not revisit that product decision — it establishes that exactly one implementation must exist, enforces the parity ADR-022 already called for, and closes the conformance gap named in the Context section. If a future product decision *does* want to change what Workspace means, that remains a product decision made through a future ADR amendment, same as any other frozen contract here.
