...
## Amendment (ADR-023 implementation, Phase 2): Workspace-folder membership excludes reserved folders

This ADR's "How they fit the existing active-view model" section described the Workspace page's folder list as coming from `VaultQuery.getRootFolders()`/`getRootPages()`, "the same query methods the sidebar's `FolderTree`/`FavoriteList` already call." That description was inaccurate as implemented: `FolderTree`'s root-level folder list has always called `VaultQuery.getVisibleRootFolders()` — a filtered method excluding `RESERVED_FOLDER_NAMES` — not the unfiltered `getRootFolders()` this ADR described the Workspace page as sharing with it. [ADR-023](./023-membership-selector-layer.md)'s implementation surfaced this as a live conformance gap: the sidebar and the Workspace page disagreed about whether Archive/Inbox/Templates/Daily Notes belong in Workspace, with no single implementation deciding the question either way.

**Resolution:** filtered wins. Workspace-folder membership excludes every reserved/system folder — matching `docs/vault/initialize/ReservedResources.ts`'s documented intent ("reserved folders... are not generic user content and should not surface in generic folder navigation") and today's actual, unchanged sidebar behavior. The Workspace collection page is corrected to match, not the reverse. This ADR's own rationale for treating Workspace as a query-defined view rather than a folder (§"Why Workspace/Favorites are views, not folders") is unaffected — only *which* query defines it changes, from unfiltered `getRootFolders()` to `MembershipSelector.getWorkspaceFolders()` (ADR-023 §4), which both the sidebar's `FolderTree` and `toCollectionPageModel`'s Workspace branch now call as their one shared implementation.

This was a documentation/implementation correction made in the course of implementing an already-accepted ADR (023), not a new design decision requiring its own alternatives analysis — the choice between "filtered" and "unfiltered" was made deliberately (see ADR-023's amended Context section) rather than defaulted to either side silently.
