...
Rendering/decorations must never mutate, normalize, duplicate, collapse, or delete Markdown source text. The syntax tree and document are authoritative at all times; a decoration only ever computes _how_ to display what's already there.
...
```html
<span class="cm-quote-marker">&gt; </span> <span class="cm-quote">Text</span>
```
...
- Marker _presentation_ may be hidden/revealed depending on engagement.
...
Known, accepted gap unrelated to either contract: a _wrapped_ quote line's continuation row carries no indent (a direct, flagged consequence of 1.4 — there is currently no in-flow mechanism reserving that space on a soft-wrapped row).
...
**Status update (2026-08-27): §7.1 step 1 has shipped for the five constructs named below** (Emphasis, StrongEmphasis, Strikethrough, Highlight, InlineCode) — the audit text immediately following describes their _pre-migration_ state and is kept for historical record, not current behavior. Current behavior: `delimitedInlineRenderer()` (`inlineLivePreviewParticipants.ts`) now takes a `markerClass` argument for these five constructs and emits `Decoration.mark({class: 'cm-marker cm-{construct}-marker cm-marker--concealed'})` for each marker range instead of `Decoration.replace({})` — real, source-backed marker spans in both engagement states, satisfying §1.2/§1.3. Autolink/Link/URL/WikiLink/Tag/Date are unaffected and still match the audit below as written (Autolink was never given a `markerClass`, still `Decoration.replace({})` — §7.1 step 2 not started).

**Status update (2026-08-27, later same day): superseded again — the `Decoration.mark` + `--concealed` CSS technique described immediately above shipped, then was replaced within the same day by `Decoration.replace()` + a dedicated `ConcealedMarkerWidget` (`inlineLivePreviewParticipants.ts`'s `delimitedInlineRenderer`, `ConcealedMarkerWidget.ts`).** Real-browser verification (not the Safari-only pass below) found the `Decoration.mark` technique could not simultaneously satisfy correct vertical geometry (CM6 `drawSelection()`'s selection-rectangle computation) and correct horizontal layout (no per-marker-length indentation) — every CSS-only variant tried fixed one axis by breaking the other, because a _mark_ decoration can only ever style the marker's real glyphs, and a real text run's width and height are governed by the same font metrics. `Decoration.replace()` with an empty, independently-styled widget sidesteps this: the widget's box is authored independently of any glyph, so `width: 0` and a normal `height` are two ordinary, uncontested declarations. See `docs/editor-architecture-decisions.md`'s `Decoration.replace()`-with-widget entry for the full investigation and real-browser verification results, and `MarkdownEditor.css`'s `.cm-marker--concealed` doc comment for the exact CSS.
This changes the answer to §1.2/§1.3/§1.5's own framing below (written when `Decoration.mark` was the _only_ way considered to get a real, source-backed, geometry-safe marker): a _widget-backed_ `Decoration.replace()` is now also a way to satisfy those same requirements for this construct family, without keeping the marker's real text in the DOM at all. Click hit-testing — the specific, named reason the audit below and §7.2's checklist worried a plain `Decoration.replace({})` was unsafe — was re-verified for the widget variant directly: a real click at the widget's own on-screen coordinate (zero-width, but still a real, positioned DOM element) correctly placed the caret and engaged the construct, with `ConcealedMarkerWidget.ignoreEvent()` returning `false` specifically so the click reaches CM6's own built-in position-resolution instead of being discarded (`WidgetType`'s documented default). The blockquote/list/heading constructs discussed elsewhere in this document are unaffected — none of them use this widget, and their own `Decoration.mark`-based or `Decoration.replace({})`-based choices stand exactly as recorded.
