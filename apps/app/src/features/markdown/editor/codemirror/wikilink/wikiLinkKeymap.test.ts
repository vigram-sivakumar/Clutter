...
import { activateAdjacent, hopLeft, hopRight } from './wikiLinkKeymap';
...
describe('hopRight / hopLeft — empirically-motivated arrow-key correction', () => {
  it('one position before an at-rest WikiLink, hops to the far boundary in one call, not one character', () => {
    // node = [2, 19); one position before its start is 1.
  ...
  it('one position after an at-rest WikiLink, hopLeft hops to the near boundary in one call', () => {
    // one position after the node's end (19) is 20.
