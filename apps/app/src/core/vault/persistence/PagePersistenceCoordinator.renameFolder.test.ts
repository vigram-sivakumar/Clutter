...
describe('PagePersistenceCoordinator rename-folder vertical slice (ADR-024, interim kind)', () => {
  ...
  it('never reparents — a rename never changes parentId even if the name matches something in another folder', async () => {
    ...
    await coordinator.enqueue('folder-child', { kind: 'rename-folder', name: 'Renamed' });
