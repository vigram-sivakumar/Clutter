...
// its real date — excluded the same way getPageDisplayLabel's own
// precedence chain excludes it, inline rather than behind a page-specific
// wrapper (isAutoGeneratedName is AutoGeneratedName.ts's one, shared,
// entity-agnostic detector; Folder has no subtype, so no exclusion needed
// there).
function entryBreadcrumbTitle(entry: Page | Folder): string {
  if (isPage(entry)) {
    const isUntitled = entry.type !== 'daily-note' && isAutoGeneratedName(entry.name);
    return isUntitled ? getPageTitlePlaceholder(entry.type) : entry.name;
  }

  return isAutoGeneratedName(entry.name) ? getFolderTitlePlaceholder() : entry.name;
}
...
/**
 * Breadcrumbs for a VaultResource (Image/PDF Resource Page) — same
 * ancestor-folder walk as buildBreadcrumbs, since a resource's parentId is
 * a real, already-known folder chain the same way a page's is. The
 * trailing crumb has no onClick (nothing to navigate to that isn't already
 * the active resource) and, unlike buildBreadcrumbs' page/folder crumb,
 * never a placeholder — a VaultResource's `name` is a real filename, never
 * user-editable/auto-generated the way a Note/Folder title can be.
 */
export function buildBreadcrumbsForResource(
  resource: VaultResource,
  vault: Vault,
  membershipSelector: MembershipSelector,
  onOpenFolder: (folderId: string) => void
): Breadcrumb[] {
  // Root-level resources have no breadcrumb trail — same policy as
  // buildBreadcrumbs.
  if (resource.parentId === null) {
    return [];
  }

  const ancestors = ancestorBreadcrumbs(
    resource.parentId,
    vault,
    membershipSelector,
    onOpenFolder
