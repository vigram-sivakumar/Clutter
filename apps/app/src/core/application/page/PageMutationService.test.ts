...
function setup(page: Page, vault?: Vault) {
  ...
  const moveService = new MoveService(resolvedVault, fileSystem);
...
async function archivePage(
  ...
  await context.service.archivePage(page.id);
  return context;
}
...
describe('PageMutationService.archivePage()', () => {
  it('sets status to archived and stamps archivedAt', async () => {
    const page = buildActivePage();
    const { vault, service } = setup(page);

    await service.archivePage(page.id);
  ...
  it('moves the page file into the Archive folder', async () => {
    const page = buildActivePage();
    const { vault, fileSystem, service } = setup(page);
  ...
  it('captures originalPath and originalParentId before moving', async () => {
    ...
    const { vault, service } = setup(page, makeVault([page], [makeArchiveFolder(), makeInboxFolder(), projects]));
  ...
  it('throws when the page is already archived', async () => {
    ...
    await expect(service.archivePage(page.id)).rejects.toThrow(/already archived/);
  ...
  it('throws for an unknown page id and does not write to disk', async () => {
    const page = buildActivePage();
    const { fileSystem, service } = setup(page);

    await expect(service.archivePage('does-not-exist')).rejects.toThrow(/Page not found/);
    expect(fileSystem.hasFileSync(page.path)).toBe(true);
    expect(fileSystem.hasFileSync(archivePathFor(page))).toBe(false);
  });
});

describe('PageMutationService.restorePage()', () => {
  it('restores the page to its original folder location', async () => {
    ...
    const { vault, fileSystem, service } = await archivePage(page, [
      makeArchiveFolder(),
      makeInboxFolder(),
      makeProjectsFolder(),
      design,
    ...
    await service.restorePage(page.id);
  ...
  it('uses the current folder path when the original folder was renamed', async () => {
    ...
    const vault = makeVault([archivedPage], [
      makeArchiveFolder(),
      makeInboxFolder(),
      makeProjectsFolder(),
      renamedDesign,
    ]);
    const { fileSystem, service } = setup(archivedPage, vault);

    await service.restorePage(archivedPage.id);
  ...
  it('throws when the restore destination path is already occupied', async () => {
    ...
    const { vault, fileSystem, service, serializer } = await archivePage(page, folders);
    ...
    await expect(service.restorePage(page.id)).rejects.toThrow(/Path already in use/);
    expect(vault.getPage(page.id)!.metadata.status).toBe('archived');
    expect(fileSystem.hasFileSync(archivePathFor(page))).toBe(true);
  });
  ...
  it('throws when the page is not archived', async () => {
    ...
    await expect(service.restorePage(page.id)).rejects.toThrow(/not archived/);
    expect(fileSystem.hasFileSync(page.path)).toBe(true);
  });
  ...
  it('throws when restoring an already restored page', async () => {
    ...
    const { service } = await archivePage(page, [
      makeArchiveFolder(),
      makeInboxFolder(),
      makeProjectsFolder(),
      design,
  ...
  it('throws for an unknown page id', async () => {
    const page = buildActivePage();
    const { service } = setup(page);

    await expect(service.restorePage('does-not-exist')).rejects.toThrow(/Page not found/);
  });
});
