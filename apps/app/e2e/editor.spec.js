...
async function nodeText(page, index) {
...
function topLevelNodes(page) {
    return page.locator('.clutter-document-body > .clutter-node');
}
test.describe('Editor', () => {
    test.beforeEach(() => {
        test.skip(true, 'Shell-only app: mount <Editor /> in App.tsx and restore editor CSS in main.tsx, then remove this skip.');
    });
    // ---------------------------------------------------------------------------
    // Typing
    // ---------------------------------------------------------------------------
    ...
    test.describe('Block-range selection', () => {
        test('dragging across two nodes selects both', async ({ page }) => {
            ...
            if (!firstBox || !secondBox)
                throw new Error('nodes not visible');
