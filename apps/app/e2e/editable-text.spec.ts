...
test.describe('EditableText', () => {
  test('clicking into the middle of the title places the caret at that position', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    expect(caretOffset).not.toBeNull();
    await expect(editableText).toHaveText(
      `${originalValue.slice(0, caretOffset!)}X${originalValue.slice(caretOffset!)}`
    );
  });

  test('typing into an empty editable starts at the natural beginning', async ({
    page,
  }) => {
    await page.goto('/');
