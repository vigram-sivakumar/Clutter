...
export function NodeEditor() {
  ...
  */
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    // 🔒 NEW ARCHITECTURE: Tab handler (indent/outdent)
    if (e.key === 'Tab') {
      if (!coordinatorRef.current) {
        console.error('[MIGRATION] Coordinator not initialized');
        return;
      }

      const result = handleTab(newEditorState, e);
      ...
      if (result.action) {
        coordinatorRef.current.execute(result);
      }
      return;
    }
    ...
    if (e.key.startsWith('Arrow')) {
      ...
      const result = handleArrow(newEditorState, e);
