 ...
 * Local-only, synchronous state management with localStorage persistence
 */
...
interface NotesStore {
  ...
  setHasHydrated: () => void;
  createNote: (
    _initialValues?: Partial<Note>,
    _setAsCurrent?: boolean
  ) => Promise<Note>;
  ...
  updateNoteMeta: (
    _id: string,
    _updates: Omit<Partial<Note>, 'content' | 'id' | 'createdAt' | 'updatedAt'>
  ) => void;
...
export const useNotesStore = create<NotesStore>()(
  persist(
    (set, get) => ({
      ...
      get currentNote() {
        // ✅ Defensive: get() can return null during rehydration
        const state = get();
      ...
      updateNote: (id, updates) => {
        ...
        if (
          ...
          hadTagsBefore &&
          updates.tags &&
          JSON.stringify(hadTagsBefore) !== JSON.stringify(updates.tags)
        ) {
          setTimeout(() => {
      ...
      restoreNote: (id) => {
        ...
        if (note?.dailyNoteDate && !updates.folderId) {
          ...
          const existingDailyNote = get().notes.find(
            (n) =>
      ...
      findDailyNoteByDate: (date) => {
        ...
        const dailyNotes = get().notes.filter(
          (n) => n.dailyNoteDate === dateStr && !n.deletedAt
        );
      ...
      createDailyNote: async (date, setAsCurrent = true) => {
        ...
        const deletedDailyNote = get().notes.find(
          (n) => n.dailyNoteDate === dateStr && n.deletedAt !== null
        );
    ...
    {
      name: 'clutter-notes-storage', // localStorage key
      version: 1,
