...
**Distinct workflows sharing a mechanism vs. a second owner of the same decision (see ADR-014's amendment):** This rule governs *decisions* — a capability has one owner if there is one place that decides *whether and how* a given kind of mutation happens for arbitrary, general-purpose callers. It does not prohibit a narrow, singular, system-triggered workflow from enqueuing directly through the Gate when that workflow is making a genuinely different decision than the one a facade method already owns — provided the write still lands through the one Gate (rule 2 is never relaxed) and the exception doesn't proliferate. `DailyNoteService.ensurePage()` is the canonical example: it decides "does today's one specific, well-known page exist yet," a singular, bootstrap-triggered, fixed-target check, not "where should this user-chosen content go" (`PageOperations.create()`'s actual, general-purpose, repeatable decision, with collision-free naming `DailyNoteService` doesn't need and a fixed path `PageOperations.create()` doesn't support). They are not two implementations of the same decision; they are two different decisions that happen to terminate at the same enforcement point, which is exactly what rule 2 requires. This is the same category as rule 2's own named exception for `VaultInitializer`'s bootstrap writes — a single, disclosed, spec-documented exception, not a growing pattern.
